...
def superpixel_SLIC(Im, region_size, ruler, iter_num, min_size):
    ...
    retval = cv2.ximgproc.createSuperpixelSLIC(img, 102, int(region_size), float(ruler)) 
 
    retval.iterate(int(iter_num))
    retval.enforceLabelConnectivity(int(min_size))
    
    segments_MSLIC=retval.getLabels()
    sup_map =  mark_boundaries(img,segments_MSLIC)
...
##    plt.show()
    ...
    print("segmented with ", np.max(segments_MSLIC)+1, " superpixels")
    return sup_map
