...
def superpixel_GraphBased(Im, scale, sigma, min_size):
    ...
    segments_fz = felzenszwalb(img, float(scale), float(sigma), int(min_size))

    sup_map =  mark_boundaries(img,segments_fz)
...
##    plt.show()
    ...
    print("segmented with ", np.max(segments_fz)+1, " superpixels")
    return sup_map
